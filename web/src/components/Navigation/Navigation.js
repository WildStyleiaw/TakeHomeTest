...
const CustomLink = ({ to, children }) => {
  ...
  const activeClassName =
    'group flex items-center px-2 py-2 text-lg leading-5 font-medium text-indigo-600 rounded-md focus:outline-none bg-gray-200 transition ease-in-out duration-150'
  const inActiveClassName =
    'group flex items-center px-2 py-2 text-lg leading-5 text-gray-600 rounded-md hover:text-indigo-600 hover:bg-gray-200 focus:outline-none focus:bg-gray-200 transition ease-in-out duration-150'
  if (matchInfo.match) {
    return (
        ...
        <nav className="flex flex-col justify-between flex-1 px-2 mt-5 bg-gray-50">
          <div className="space-y-2">
            <CustomLink to={routes.dashboard()}>
              <svg
                className="w-6 h-6 mr-3 transition duration-150 ease-in-out"
